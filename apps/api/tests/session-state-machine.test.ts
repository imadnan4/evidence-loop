...
function prepareStartedSession(service: TextCheckInSessionService) {
  const session = service.createSession(learner, sessionInput());
  service.showPolicy(learner, { sessionId: session.id, idempotencyKey: "shown-1" });
  service.acknowledgePolicy(learner, { sessionId: session.id, policyVersionId: "version-1", idempotencyKey: "ack-1" });
  return service.start(learner, { sessionId: session.id, policyVersionId: "version-1", mode: "text", idempotencyKey: "start-1" });
}

test("a typed check-in requires learner-visible policy acknowledgement before deterministic questions start", () => {
  const { service } = setup();
  const session = service.createSession(learner, sessionInput());

  assert.throws(
    () => service.start(learner, { sessionId: session.id, policyVersionId: "version-1", mode: "text", idempotencyKey: "start-before-policy" }),
    (error: unknown) => error instanceof SessionError && error.code === "INVALID_STATE",
  );
  const briefing = service.showPolicy(learner, { sessionId: session.id, idempotencyKey: "shown-1" });
  ...
  const started = service.start(learner, { sessionId: session.id, policyVersionId: "version-1", mode: "text", idempotencyKey: "start-1" });
...
test("pause and resume preserve the outstanding typed question without spending more budget", () => {
  const { service } = setup();
  const started = prepareStartedSession(service);
  const paused = service.pause(learner, { sessionId: started.session.id, idempotencyKey: "pause-1" });
  assert.equal(paused.state, "paused");
  assert.equal(paused.questions_asked, 1);
  const resumed = service.resume(learner, { sessionId: paused.id, idempotencyKey: "resume-1" });
  assert.equal(resumed.state, "in_progress");
  assert.equal(resumed.questions_asked, 1);
  assert.throws(
    () => service.submitTextResponse(learner, { sessionId: resumed.id, questionId: "id-not-a-question", canonicalText: "reasoning", editedText: null, idempotencyKey: "bad-question" }),
    (error: unknown) => error instanceof SessionError && error.code === "NOT_FOUND",
  );
});

test("the finite question budget completes the session and returns a canonical text receipt", () => {
  const { service } = setup();
  let started = prepareStartedSession(service);
  let result = service.submitTextResponse(learner, {
  ...
  result = service.submitTextResponse(learner, {
  ...
  const receipt = service.getReceipt(learner, result.session.id);
  ...
  assert.deepEqual(service.getTimeline(learner, result.session.id).map((event) => event.action), [
...
test("writes replay with the same idempotency key and reject a changed request", () => {
  ...
  const first = service.createSession(learner, sessionInput("create-replay"));
  const replay = service.createSession(learner, sessionInput("create-replay"));
  assert.equal(replay.id, first.id);
  assert.throws(
    () => service.createSession(learner, { submissionId: "submission-2", idempotencyKey: "create-replay" }),
    (error: unknown) => error instanceof SessionError && error.code === "IDEMPOTENCY_CONFLICT",
  );

  service.showPolicy(learner, { sessionId: first.id, idempotencyKey: "shown-1" });
  service.acknowledgePolicy(learner, { sessionId: first.id, policyVersionId: "version-1", idempotencyKey: "ack-1" });
  const started = service.start(learner, { sessionId: first.id, policyVersionId: "version-1", mode: "text", idempotencyKey: "start-1" });
  ...
  assert.equal(service.submitTextResponse(learner, response).response.id, service.submitTextResponse(learner, response).response.id);
  assert.throws(
    () => service.submitTextResponse(learner, { ...response, canonicalText: "A different explanation." }),
    (error: unknown) => error instanceof SessionError && error.code === "IDEMPOTENCY_CONFLICT",
  );
});

test("trusted resolution rejects another learner, unpublished/mismatched versions, and fabricated fragment provenance", () => {
  const { service: privateService } = setup({ contexts: [trustedContext({ learnerId: outsider.userId })] });
  assert.throws(
    () => privateService.createSession(learner, sessionInput()),
    (error: unknown) => error instanceof SessionError && error.code === "NOT_FOUND",
  );
  ...
  for (const context of [
    ...
    assert.throws(
      () => service.createSession(learner, sessionInput()),
  ...
  const session = service.createSession(learner, sessionInput());
  assert.throws(
    () => service.showPolicy(outsider, { sessionId: session.id, idempotencyKey: "outside" }),
    (error: unknown) => error instanceof SessionError && error.code === "FORBIDDEN",
  );
});

test("the time budget completes the session without accepting a late response, while pauses do not consume it", () => {
  let now = "2026-07-18T12:00:00.000Z";
  const { service } = setup({ now: () => now });
  const started = prepareStartedSession(service);
  now = "2026-07-18T12:01:00.000Z";
  service.pause(learner, { sessionId: started.session.id, idempotencyKey: "pause-time" });
  now = "2026-07-18T12:11:00.000Z";
  service.resume(learner, { sessionId: started.session.id, idempotencyKey: "resume-time" });
  now = "2026-07-18T12:13:00.000Z";
  assert.throws(
    () => service.submitTextResponse(learner, { sessionId: started.session.id, questionId: started.question.id, canonicalText: "Late response", editedText: null, idempotencyKey: "late-response" }),
    (error: unknown) => error instanceof SessionError && error.code === "INVALID_STATE" && error.message.includes("time budget"),
  );
  const receipt = service.getReceipt(learner, started.session.id);
  assert.equal(receipt.responses.length, 0);
  assert.equal(receipt.session.completed_at, "2026-07-18T12:13:00.000Z");
});

test("the learner can request a human follow-up instead of continuing indefinitely", () => {
  const { service } = setup();
  const started = prepareStartedSession(service);
  const requested = service.requestHumanFollowUp(learner, { sessionId: started.session.id, idempotencyKey: "follow-up-1" });
  assert.equal(requested.state, "human_follow_up");
  const receipt = service.getReceipt(learner, requested.id);
...
test("unsafe judgment fields and voice mode are rejected from the typed-session boundary", () => {
  const { service } = setup();
  assert.throws(
    () => service.createSession(learner, { ...sessionInput(), cheatingLikelihood: 0.8 } as never),
    (error: unknown) => error instanceof SessionError && error.code === "INVALID_REQUEST",
  );
  const started = prepareStartedSession(service);
  assert.throws(
    () => service.start(learner, { sessionId: started.session.id, policyVersionId: "version-1", mode: "voice" as never, idempotencyKey: "voice-not-f04" }),
    (error: unknown) => error instanceof SessionError && error.code === "INVALID_REQUEST",
  );
});
